...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    if ((textField == self.packageDescription) || (textField == self.trackingNumber)) {
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([[segue identifier] isEqualToString:@"ReturnInput"]) {
        if ([self.packageDescription.text length] || [self.trackingNumber.text length] || [_shippingProvider length]) {
            Package *newPackage;
            newPackage = [[Package alloc] initWithName:self.packageDescription.text tracking:self.trackingNumber.text shipper:_shippingProvider];
